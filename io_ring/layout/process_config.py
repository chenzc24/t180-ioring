...
# -*- coding: utf-8 -*-
"""
Process Node Configuration - T180 (180nm) specific
...
def get_process_node_config() -> Dict[str, Any]:
    """
    Get configuration for T180 process node
    Loads from JSON file if available, falls back to hardcoded config

    Returns:
        Configuration dictionary for T180 process node
    """
    ...
    if device_config:
        ...
        if "layout_params" in device_config:
            ...
            config.update({
                ...
                "pad_offset": layout_params.get("pad_offset", 10),
            })
...
def get_device_offset(device_type: str) -> float:
    """
    Get device offset based on device type for T180 process
    ...
    # Check for prefix match (device_type may have suffix)
...
def get_template_file_paths() -> list:
    """
    Get possible template file paths for T180 process node

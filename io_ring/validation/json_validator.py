...
def validate_config(config: Dict[str, Any]) -> bool:
    ...
    for i, instance in enumerate(instances):
        ...
        if device.startswith('PDDW0412SCDG'):
...
def get_config_statistics(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    digital_ios = [inst for inst in instances if inst.get('device') == 'PDDW0412SCDG']
...
def main() -> None:
    """CLI entry point used by scripts/validate_intent.py."""
    if len(sys.argv) < 2:
    ...
    if not config_path.exists():
        print(f"[ERROR] Error: File not found: {config_path}")
        sys.exit(2)
    ...
    try:
        ...
        print(f"[ERROR] Error: Failed to read file: {e}")
        sys.exit(2)
